...
| Authentication      | Laravel Fortify | bundled / 1.x            | Headless authentication back-end (login, registration, password reset, email verification) | Install `laravel/fortify` and register `FortifyServiceProvider`; publish the config (`fortify.php`) and the `app/Actions/Fortify/` action stubs. Because this project uses Inertia (not Blade views), disable Fortify's built-in view responses and wire the Inertia page components (`Login`, `Register`) to Fortify's POST endpoints manually. Session-cookie auth via the `web` guard — no API tokens needed. |
| Testing Framework   | Pest PHP        | 3.x                      | Unit and feature test runner                                  | Installed as a dev dependency (`pestphp/pest`, `pestphp/pest-plugin-laravel`). Test files live under `tests/Feature/` and `tests/Unit/` and mirror the `app/` directory structure. Feature tests run against PostgreSQL (not SQLite) to honour `CHECK` constraints and `timestamptz` semantics; unit tests use an in-memory SQLite database via `RefreshDatabase`. |
